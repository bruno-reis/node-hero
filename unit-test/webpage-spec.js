...
describe('the webpage module', () => {
  it('saves the content', function *() {
    ...
    const writeFileStub = this.sandbox.stub(fs, 'writeFile', (filePath, fileContent, cb) => cb(null))

    const requestStub = this.sandbox.stub(request, 'get', (url, cb) => cb(null, null, content))
